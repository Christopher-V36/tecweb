$(document).ready(function(){
    ...
    $('#search').keyup(function(e){
        if($('#search').val()){
            ...
            if(search){
                $.ajax({
                    url:'http://localhost/tecweb/practicas/p12/product_app/backend/product-search.php',
                    type: 'POST',
                    data:{search},
    ...
    $('#product-form').submit(function(e){
        ...
        console.log("Datos enviados:", postData);
                const url = edit === false ? 'http://localhost/tecweb/practicas/p12/product_app/backend/product-add.php' : 'http://localhost/tecweb/practicas/p12/p12/product_app/backend/producto_edit.php';
                if (edit) {
                    postData.id = id;
                }
    ...
    function listar(){
        $.ajax({
            url:'http://localhost/tecweb/practicas/p12/product_app/backend/product-list.php',
            type: 'GET',
            success: function (response){
    ...
    $(document).on('click', '.eliminar',function(){
        if(confirm('¿Estás seguro de querer eliminar el elemento?')){
            ...
            $.post('http://localhost/tecweb/practicas/p12/product_app/backend/product-delete.php',{id: id},function(response) {
                console.log(id);
    ...
    $(document).on('click', '.editar', function() {
        if (confirm('¿Estás seguro de querer editar el elemento?')) {
            ...
            let id = fila.find('td').first().text();
            edit=true;
            $.post('http://localhost/tecweb/practicas/p12/product_app/backend/update_producto.php', {id: id}, function(response) {
                console.log("Respuesta del servidor:", response);
